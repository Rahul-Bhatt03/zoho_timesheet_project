...
class ZohoTimesheetImport implements ToCollection
{
    protected $teamName;
    ...
    protected array $fieldMappings = [
        ...
        'application'          => ['application', 'app', 'project_name'],
        ...
        'team_name'            => ['teamname', 'team_name', 'team', 'log_owner'],
    ...
    public function __construct(string $teamName,string $fileExtension=null)
    {
...
private function isCSVfile(): bool
{
    ...
    public function collection(Collection $rows)
    {
        ...
        // Find the header row by looking for "Item ID" in column A (index 0) for both csv and .xlsx
        $headerRowIndex = null;
        $headers = null;

        // search through first 15 rows to find headers
        for ($i = 0; $i < min(15, $rows->count()); $i++) {
            $row = $rows->get($i);
            if (!$row || $row->isEmpty()) {
                continue;
            }
            $firstCell = $row->first();
            if ($this->isHeaderRow($row, $firstCell)) {
                $headerRowIndex = $i;
                $headers = $row;
                Log::info("headers row found on index:{$headerRowIndex}");
                break;
            }
        }

        if (!$headers || $headerRowIndex === null) {
            Log::warning("No header row found with 'Item ID' pattern.");
            return;
        }
        ...
        // Process data rows (skip all rows up to and including the header row)
        $dataRows = $rows->slice($headerRowIndex + 1);

        foreach ($dataRows as $rowIndex => $row) {
            $actualRowIndex = $headerRowIndex + 1 + $rowIndex; // Calculate actual row number
            Log::info("=== PROCESSING ROW {$actualRowIndex} ===");

            if ($row->filter()->isEmpty()) {
                Log::info("Skipping empty row {$actualRowIndex}");
                continue;
            }
            ...
            try {
                ...
                Log::error("Failed to create entry for row {$actualRowIndex}: " . $e->getMessage());
    ...
    private function isHeaderRow($row, $firstCell): bool
    {
        if (!$firstCell) {
            return false;
        }
        $normalizedFirstCell = strtolower(trim($firstCell));

        // primary check i.e looking for item id variation in 1st column
        $itemIdVariations = [
            'item_id',
            'item id',
            'itemid',
        ...
        foreach ($itemIdVariations as $variation) {
            if ($normalizedFirstCell === $variation) {
                Log::info("headers detected at row index: {$firstCell}");
                return true;
            }
        }
        ...
        $headerMatchCount = 0;
        $totalNonEmptyColumns = 0;

        foreach ($row as $cell) {
            if ($cell && trim($cell) !== '') {
                $totalNonEmptyColumns++;
                $normalizedCell = strtolower(trim($cell));

                foreach ($headerTerms as $term) {
                    if (strpos($normalizedCell, $term) !== false) {
                        $headerMatchCount++;
                        break;
                    }
                }
        ...
        // If we have at least 5 non-empty columns and 40% contain header terms
        if ($totalNonEmptyColumns >= 5 && ($headerMatchCount / $totalNonEmptyColumns) >= 0.4) {
            Log::info("Header detected by pattern analysis. Matches: {$headerMatchCount}/{$totalNonEmptyColumns}");
            return true;
        }
